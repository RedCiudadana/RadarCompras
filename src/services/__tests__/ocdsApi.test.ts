 ...
 * estos tests fallarán. Basado en:
 *   - docs.json /release/search → HeaderRelease schema
 *   - docs.json components/schemas/Releases
 *   - docs.json components/schemas/Pagination
 */
...
describe('OCDSApi.searchReleases', () => {
  ...
  // --- Validación de estructura (fallan si el contrato de la API cambia) ---
  ...
  it('Releases schema: cada release tiene los campos obligatorios de docs.json', async () => {
    ...
    // Campos obligatorios según docs.json Releases schema
  ...
  it('Pagination schema: links.next es string o null', async () => {
    mockFetchSuccess(mockHeaderRelease);
    const { hasMore } = await OCDSApi.searchReleases();
    // links.next = null → hasMore false
    expect(hasMore).toBe(false);
  });

  it('Pagination schema: hasMore es true cuando links.next no es null', async () => {
    mockFetchSuccess({
      ...mockHeaderRelease,
  ...
  it('Releases.tag: es un array de strings (tag schema)', async () => {
    mockFetchSuccess(mockHeaderRelease);
    const { data } = await OCDSApi.searchReleases();
  ...
  // --- Mapeo de parámetros hacia la API ---

  it('envía Anio y Mes por defecto con la fecha actual', async () => {
    mockFetchSuccess(mockHeaderRelease);
  ...
  it('usa filters.year → Anio y filters.month → Mes cuando se proveen', async () => {
    mockFetchSuccess(mockHeaderRelease);
  ...
  it('envía q cuando filters.keyword está definido', async () => {
    mockFetchSuccess(mockHeaderRelease);
    await OCDSApi.searchReleases({ keyword: 'medicamentos' });

    const calledUrl = vi.mocked(fetch).mock.calls[0][0] as string;
    expect(calledUrl).toContain('q=medicamentos');
  });

  it('envía Estatus_concurso=1 (Vigente) siempre', async () => {
    mockFetchSuccess(mockHeaderRelease);
    await OCDSApi.searchReleases();
  ...
  it('retorna data vacía cuando releases es undefined en la respuesta', async () => {
    mockFetchSuccess({ links: { next: null, prev: null } }); // sin releases
    const result = await OCDSApi.searchReleases();
...
describe('OCDSApi.filterReleases', () => {
  ...
  it('filtra por minAmount', () => {
    const match = OCDSApi.filterReleases(releases, { minAmount: 100_000 });
    expect(match).toHaveLength(1);

    const noMatch = OCDSApi.filterReleases(releases, { minAmount: 1_000_000 });
    expect(noMatch).toHaveLength(0);
  });

  it('filtra por maxAmount', () => {
    const match = OCDSApi.filterReleases(releases, { maxAmount: 1_000_000 });
    expect(match).toHaveLength(1);

    const noMatch = OCDSApi.filterReleases(releases, { maxAmount: 100 });
    expect(noMatch).toHaveLength(0);
  });
