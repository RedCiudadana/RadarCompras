...
export const MarketContext: React.FC<MarketContextProps> = ({
  ...
  return (
    <section>
      <h2 className="flex items-center gap-2 text-sm font-semibold text-rc-primary uppercase tracking-wide mb-1">
        <TrendingUp className="w-4 h-4" />
        Contexto de mercado (Últimos {MONTHS_LOOKBACK} meses)
      </h2>
      <p className="text-xs text-rc-text-subtle mb-3">
      ...
      {loading && (
        <div className="bg-white border border-rc-border rounded-lg px-4 py-6 flex items-center justify-center gap-2 text-sm text-rc-text-subtle">
      ...
      {!loading && error && (
        <div className="bg-white border border-rc-border rounded-lg px-4 py-6 text-center text-xs text-rc-text-subtle">
          No se pudo cargar el contexto de mercado.
        </div>
      )}

      {!loading && !error && comparables.length === 0 && (
        <div className="bg-white border border-rc-border rounded-lg px-4 py-6 text-center text-sm text-rc-text-subtle">
      ...
      {!loading && !error && comparables.length > 0 && (
        <>
            ...
            {comparables.map(release => {
              ...
              return (
                <a
                  ...
                  className="flex flex-col gap-1 px-4 py-3 border-b border-rc-border last:border-b-0 hover:bg-rc-surface transition-colors group"
                >
                  <div className="flex items-start gap-2">
                    <span className="flex-1 min-w-0 text-sm font-medium text-rc-text-base group-hover:text-rc-primary transition-colors line-clamp-2">
                      {title}
                    </span>
                    <ExternalLink className="w-3.5 h-3.5 shrink-0 mt-0.5 text-rc-text-subtle opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <div className="flex items-center gap-3 text-xs">
                    <span className="shrink-0 text-rc-text-subtle tabular-nums">
                      {formatDate(date)}
                    </span>
                    <span className="shrink-0 font-semibold text-rc-accent">
                      {formatCurrency(amount, currency)}
                    </span>
                    <span className="flex-1 min-w-0 truncate text-rc-text-muted">
                      {supplier}
                    </span>
                    <span className="shrink-0 text-rc-text-subtle">
                      {status}
                    </span>
                  </div>
                </a>
          ...
          <p className="text-xs text-rc-text-subtle mt-2">
